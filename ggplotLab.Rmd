...
```{r}
ggplot(heights, aes(height)) + geom_histogram()
...
```{r}
ggplot(heights, aes(x = height, colour = sex)) + geom_density()

# tried with fill for next part
...
ggplot(heights, aes(x = height, fill = sex)) + geom_density(alpha = 1/2) 
...
```{r}
ggplot(heights, aes(x = height)) + geom_boxplot()


ggplot(heights, aes(x = height, y = sex)) + geom_boxplot()

ggplot(heights, aes(x = height, color = sex)) + geom_boxplot()
...
heights %>%
  ...
  summarize(
    mean_height = mean(height),
    median_height = median(height)
) %>% ggplot( aes(x = sex, y = mean_height)) + geom_col()
...
```{r}
ggplot(votedata %>% filter(year == 2000)) + geom_col(aes(x = demVote, y = state))
